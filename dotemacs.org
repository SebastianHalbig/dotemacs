...
;; Copyright (C) 2026 Sebastian Halbig
...
#+begin_src emacs-lisp
(use-package werner-colours
   :ensure nil
  :load-path "~/.emacs.d/elpaca/repos/werner-colours/")
#+end_src
*** The theme itself
...
(use-package org-gtd
  ...
  (org-gtd-areas-of-focus '("Family" "Research" "Teaching" "Grants and Applications" "Admin"))
  (org-gtd-organize-hooks '(org-gtd-set-area-of-focus org-set-tags-command))
  (org-gtd-directory (car org-agenda-files))
  (org-gtd-inbox (concat (file-name-as-directory org-gtd-directory) "inbox"))
  (org-gtd-archive-location '(lambda() (concat
                                        (file-name-as-directory org-gtd-directory)
                                        "archive.org::datetree/")))
  (org-todo-keywords '((sequence "TODO" "NEXT" "WAIT" "|" "DONE" "CANCEL")))
...
The [[https://github.com/rougier/nano-calendar][ nano-calendar]] package shows a calendar with a heat meat depending on tasks for a given day.
...
(use-package nano-calendar
  ...
  :bind (:map ergoemacs-override-keymap ("C-c d d" . nano-calendar)))
#+end_src
*** nano-agenda
...
#+end_src
** Org roam
[[https://github.com/org-roam/org-roam?tab=readme-ov-file][Org roam]] provides a digital variant of Luhman's Zettelkasten
#+begin_quote
...
(use-package org-roam
  :ensure (:url "https://github.com/org-roam/org-roam")
  :custom
...
(setq professional-mail-signature (concat
                      "Sebastian Halbig — https://sebastianhalbig.github.io\n"
                      "BBB home room: "
...
(add-to-list 'mu4e-contexts
             (make-mu4e-context
              ...
              :vars `((user-mail-address . "halbigs@mathematik.uni-marburg.de")
                      ...
                      (smtpmail-smtp-server . "smtp.mathematik.uni-marburg.de")
                      (smtpmail-stream-type . starttls)
                      (smtpmail-smtp-service . 587))))
#+end_src
*** Marburg
...
(ii) ``open quotation and text* -> ``open quotation and text''
...
(defun my-toggle-tex-quote-region ()
  ...
  (if (use-region-p)
      (save-excursion
        (let* ((beg-pos (min (mark) (point)))
               (end-pos (max (mark) (point)))
               (start-quoted nil)
               (end-quoted nil))
          (if (> beg-pos 1)
              (progn
...
(use-package biblio-zbmath
  ...
  :config
  (put 'biblio-zbMath-query-history 'history-length 50)
  (with-eval-after-load 'savehist
  (add-to-list 'savehist-additional-variables 'biblio-zbMath-query-history))
  )
#+end_src
...
(use-package gptel
  ...
  :init
  (setq
   gptel-model 'ministral-3:3b
   gptel-backend (gptel-make-ollama "Ollama"  ;Ollama does run locally.
                   :host "localhost:11434"
                   :stream t
                   :models '((ministral-3:3b
                              :description "The latest small model provided by Mistral AI"
                              :capabilities (tools)
                              )
                             (ministral-3:8b
                              :description "The latest medium-small model provided by Mistral AI"
                              :capabilities (tools)
                              )
                             (deepseek-r1
                              :description "The 8b parameter model of DeepSeek"
                              :capabilities (tools thinking))))
   gptel-directives '((default .  "You are an AI writing assistant living inside emacs. Your job is to help in the creation of academic papers written in British English. Be precise and concise."))
   )

  (defvar math-rag-python
    (expand-file-name "~/.ollama/ingest/.venv/bin/python"))

  (defvar math-rag-engine
    (expand-file-name "~/.ollama/ingest/query_engine.py"))

  (defvar math-rag-workspace
    (expand-file-name "~/.ollama/ingest/rag_workspace"))

  (defun math-rag-query (query)
    "Run the local math RAG engine inside its venv and return JSON."
    (let* ((cmd (format
                 "%s %s --workspace %s --query %S --json"
                 math-rag-python
                 math-rag-engine
                 math-rag-workspace
                 query))
           (output (shell-command-to-string cmd)))
      (json-parse-string output :array-type 'list)))

  (gptel-make-tool
   :name "search_math_library"
   :function #'math-rag-query
   :description "Search your local mathematical research library. Call with '<Type>:<Question>'.
Use types: Definition, Theorem, Proof, or Summarise based on context.
Use this when the user asks for specific mathematical concepts or research details.
"
   :args '((:name "query"
                  :type "string"
                  :description "The mathematical concept or question to search for."))
   :category "research")


  (gptel-make-preset "agentic-writer"
    :description "Preset for a LaTeX writing assistant."
    :backend (gptel-get-backend "Ollama") ;; Assumes an Ollama backend is defined
    :model 'ministral-3:3b
    :tools '("search_math_library")
    :system "You are a writing assistant for mathematical research. Your main duty is text expansion based on structured requests of the form: <type>:<structure>.

When provided with a request, use the 'search_math_library' tool to find relevant definitions, theorems, or proofs. Avoid conversational preambles like 'A Hopf algebra is...'.

Strict Requirements:
1. Language: British English.
2. Punctuation: Use Oxford commas consistently.
3. Output: Provide polished LaTeX-ready text. Use environemnts and functions appropriately.
4. Logic: Use the tool with the format 'Type:Concept' before expanding the text."
    :temperature 0.7)
  ...
  (gptel-make-preset 'proofreader
    ...
    :system "Act as a proofreading expert tasked with correcting grammatical errors in a given text. Your job is to meticulously analyse the text, identify any grammatical mistakes, and make the necessary corrections to ensure clarity and accuracy. This includes checking for proper sentence structure, punctuation, verb tense consistency, and correct usage of words. Additionally, provide suggestions to enhance the readability and flow of the text. The goal is to polish the text so that it communicates its message effectively and professionally. The language, unless specified otherwise is British English. Oxford commas are being used."
    :temperature 0.7)

  (gptel-make-preset 'zen
    :description "Preset for getting things done"
    :backend "Ollama"
    :model 'deepseek-r1\(\)
    ;;:tools '("read_buffer" "spell_check" "grammar_check")
    :system "You are an AI-tool livinig in an org-mode buffer and assisting with a Getting Things Done (GTD) weekly review process.
The output should use headers and tags. Follow these two steps:

### Step 1: Ask Questions
Ask the following five questions to gather information for the review:
1. How many tasks did you complete this week?
2. How many tasks were deferred to next week or later?
3. Were there any tasks that took longer than expected? If so, which ones?
4. Did you prioritise any unimportant tasks over important ones? Please provide examples.
5. Where do you feel most of your time was spent this week?

After these questions wait for my response. Do not continue with step 2 on your own. I will answer the questions. After my answer decide whether you need more information on one of these questions or if you are prepared to write the report. If you need more information come up with follow up questions and iterate this process at most 3 times.
If you are ready to write the report continue with step 2.

### Step 2: Generate Report
Based on the answers, generate a report with the following sections:

#### a) Statistics of the Week
- Use headers and tags to summarise the number of tasks completed and deferred.

#### b) Areas of Improvement
- Analyze where time might have been wasted or if unimportant tasks were prioritised.

#### c) Summary and Recommendations
- Summarise the week's productivity and offer recommendations for the upcoming week."
    :temperature 0.7)
...
Rainbow mode displays colours as background behind their hex strings.
#+begin_quote
...
A small internal transient menu to explore elpaca.
#+begin_src emacs-lisp
(use-package elpaca-transient
  :after (elpaca transient)
  :ensure nil
  :load-path "~/.emacs.d/elpaca/repos/elpaca-transient/"
)
#+end_src
