...
If one is in a maths context and the previous character was a space, it inserts `\\blank'. If the previous word matches the regexp `\\[q]+uad ' it either changes `\\quad ' to `\\qquad ' or, if it was `\\qquad ', it changes `\\qquad ' to `\\qquad\\quad '."
  (interactive)
  (if (and (= (char-before) ? ) (texmathp))
      (let ((cur-word (save-excursion (backward-word)
        ...
        (cond
         ...
         ((string= cur-word "qquad")
          (backward-char)
          (insert "\\quad"))
         ((save-excursion (backward-char 2) (looking-at-p "\\\\ "))
          (backward-char)
          (insert "quad"))
         (t
          (insert "\\blank")))
