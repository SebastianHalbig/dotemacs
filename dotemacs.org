...
We use [[https://github.com/awth13/org-appear][org-appear]] in order to dynamically dispaly certain symbols, such as emphasis-markers, depending on the position of the coursor.
#+begin_quote
...
(setq org-capture-templates
      `(("i" "Inbox" entry  (file+headline ,org-gtd-inbox "Miscellaneous")
         ,(concat "* %?\n"
                  "/Entered on/ %U"))
        ("m" "Meeting" entry  (file+datetree ,org-gtd-inbox)
         ,(concat "* Metting with %?\n at <%<%Y-%m-%d %a %^{Time}>>\n"
                  "/Entered on/ %U")
         :time-prompt t)
        ("l" "Inbox with link" entry (file+headline ,org-gtd-inbox "Miscellaneous")
         ,(concat "* %?\n %a"
                  "/Entered on/ %U"
                  "%i")
                  :kill-buffer t)
        ("@" "Inbox [mu4e]" entry (file+headline ,org-gtd-inbox "Mails")
         ,(concat "*  Reply to \"%a\" %?\n"
                  "/Entered on/ %U"))))
...
* Mu4e
 ...
 (use-package mu4e
   ...
   :bind (:map ergoemacs-override-keymap ("C-c m" . mu4e))
   (:map mu4e-headers-mode-map ("g" . mu4e-headers-mark-for-tag ))
   :custom
   <<mu4e-custom>>
   :config
   <<mu4e-config>>
...
(setq personal-mail-signature (concat
                      "Sebastian Halbig — https://sebastianhalbig.github.io/main.html\n"
                      "Philipps-Universität Marburg — Research Group Algebraic Lie Theory\n"
                      "Hans-Meerwein-Straße 6 Room 04C35, 35043 Marburg\n"
                      "BBB home room: https://webconf.hrz.uni-marburg.de/d/seb-omn-wqu-r6k"))

;; Better refiling and spam handling
<<mu4e-refile>>
<<my-mu4e-spam-folder>>
...
(add-to-list 'mu4e-contexts
             (make-mu4e-context
              ...
              :vars `((user-mail-address . "Sebastian.Halbig@uni-marburg.de") ;;"halbigs@mathematik.uni-marburg.de"
                      (user-full-name . "Sebastian Halbig")
                      (message-signature . ,personal-mail-signature)
...
#+end_src
** TODO Read :INACTIVE:BINDING:FACE:
...
(set-face-attribute 'mu4e-header-highlight-face nil :inherit 'nano-salien-i)
...
#+end_src
** Setup :PERSONAL:BINDING:INACTIVE:
...
#+end_src
** Spam handling :INACTIVE:BINDING:
...
#+end_src
** Layout for mu4e :INACTIVE:HOOK:BINDING:
*** Headers
...
**** TODO Colorisation of recent emails :INACTIVE:

#+NAME: my-mu4e-headers-colorise
#+begin_src emacs-lisp :tangle no
...
(defun my-mu4e-headers-colorise (item msg)
  ...
  (let* ((recent (* 5 60)) ;; 5 minutes
    ...
    (if (and (< delta recent) unread)
        (propertize item 'face 'lambda-focus)
      item)))
...
**** Thread count (custom info) :INACTIVE:
