...
(straight-use-package
  '(latex-goodies-for-emacs :type git :host github :repo "SebastianHalbig/latex-goodies-for-emacs"))
...
(set-fontset-font t '(#xe000 . #xffdd)
                  (font-spec :name "RobotoMono Nerd Font"
                             :size (if(>=(window-pixel-height) 1700)
                                      28
                                      14)) nil)
...
(use-package visual-fill-column
  ...
  :hook  (((LaTeX-mode org-mode) . visual-fill-column)
...
(setq corfu-cycle t                ; Enable cycling for `corfu-next/previous'
      ...
      corfu-auto-delay 20.0        ; Delay before auto-completion shows up
...
(setq mu4e-signature (concat
                      "Sebastian Halbig — https://sebastianhalbig.github.io/outreach/\n"
...
(setq mu4e-bookmarks
      '((:name "Unread"
        ...
        (:name "Secondary"
               :key ?e
               :hide-unread t
               :query "tags:secondary")

        (:name "Action"
               :key ?a
               :hide-unread t
               :query "tags:untreated")
        ))
#+end_src
...
CdLaTeX makes a lot of editing work easier. We initialise it here.
...
We set up autocompletions and smart way of dealing with inputs of "$" and "<tab>"

#+begin_src emacs-lisp

(use-package latex-goodies-for-emacs
  :ensure reftex
  :init
  (yas-minor-mode 1) ; We use yas and laas auto-completion
  (laas-mode 1)      ; We use laas features (for now)
  :config
    (generate-expanded-delimiters-lists) ; Setup all environments.
  :bind (:map LaTeX-mode-map
              ("$" . cycle-texmath)
              ("<return>" . smart-return)))
...
In order to prettify symbols, we use fira code.

#+begin_src emacs-lisp
(defun my-LaTeX-variable-pitch ()
...
(use-package fira-code-mode
  ...
  :hook
  (LaTeX-mode . my-LaTeX-variable-pitch)
  (LaTeX-mode . fira-code-mode)
  :config
  (fira-code-mode-set-font)
  (require 'latex-goodies-for-emacs)
  (latex-symbols-prettify) ;; this should not be part of the package.
  (latex-regexp-prettify) ;; as well as this function.
  )
