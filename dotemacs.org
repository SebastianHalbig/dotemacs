...
This document is a literate configuration for GNU Emacs written in [[https://orgmode.org/][org]] format. It is inspired by the configurations:
...
- [[https://gitlab.com/slotThe/dotfiles/-/tree/master/emacs][Tony Zorman]] (from whom I learnt that I want emacs).

To extract the init.el and early-init.el files from this document, execute ~C-c C-v C-t~ ([[org-babel-tangle]]).
...
Aside frome these standard tags, there are two more tags, indicating some kind of unintended behaviour.
...
The early-init code is taken almost verbatim from [[https://protesilaos.com/emacs/dotemacs#h:7b7b5898-09f7-4128-8af0-4041f67cb729][Prot's config]] and combined with the early init of [[https://github.com/rougier/dotemacs/blob/master/dotemacs.org][Rougier]].
...
A short summary of its functionality take from its documentation is as follows:
...
It provides a complete overhault of the standard keybindings and therefore has to be loaded prior to any other module.
Additionally, we combine it multilingual setup.
...
** Further general layout settings
...
** Lambda modeline
The modeline is [[https://github.com/Lambda-Emacs/lambda-line][lambda-line]]. We customise it at bit.
...
(use-package lambda-line
  ...
  (lambda-line-vc-symbol "  ") ; I more specious variant of the "git-branch" symbol
...
We will use the power of company but hide it under a cape.
...
#+end_src
** Dash
[[https://github.com/magnars/dash.el][Dash]] makes working with lists better.
#+begin_src emacs-lisp
(use-package dash)
#+end_src
** TODO Embark :TEST:
...
(use-package orderless
  ...
  :custom-face
  (orderless-match-face-0 ((t (:foreground "#71aa89" :background nil)))) ; verdiris-green
  (orderless-match-face-1 ((t (:foreground "#97b578" :background nil)))) ; emerald-green
  (orderless-match-face-2 ((t (:foreground "#98a760" :background nil)))) ; pistachio-green
  (orderless-match-face-3 ((t (:foreground "#86996d" :background nil)))) ; grass-green
  :custom
...
We activate /visual-line-mode/ globally. Additionally, we use the package "[[https://codeberg.org/joostkremers/visual-fill-column][visual fill column]]" to vastly increase readability in /org/ and /LaTeX/ buffers.
...
** YASnippet
We use yasnippet in Latex mode
...
** Miniframe :INACTIVE:MODE:FACE:BUGFIX:
...
(defvar mu4e-tag-history '("RESEARCH" "TEACHING" "ORGANISATIONAL" "INFO" "INVOICE" "SAVE"  "PENDING")
  "Mu4e tag history list.")

(add-to-list 'savehist-additional-variables 'mu4e-tag-history)

(put 'mu4e-tag-history 'history-length 7)
...
* Math-mode :INACTIVE:DEFER:MODE:BINDING:
...
* TODO Python :INACTIVE:DEFER:MODE:
...
(use-package zotra
  ...
  (setq zotra-cli-command '("~/.emacs.d/zotra-cli/bin/index.js"))
  (setq zotra-after-get-bibtex-entry-hook '(finalise-entry
                                            zotra-download-attachment-for-current-entry
                                            bibtex-fill-entry))
  ;;(remove-hook 'zotra-after-get-bibtex-entry-hook 'bibtex-clean-entry)
  ;; (add-hook 'zotra-after-get-bibtex-entry-hook 'zotra-download-attachment-for-current-entry)
  ;; (add-hook 'zotra-after-get-bibtex-entry-hook 'bibtex-clean-entry -100 t)
  ;; (ebib-index-mode-map
...
(use-package org
  ...
  (setq org-todo-keywords
        '((sequence "TODO(t)" "NEXT(n)" "|" "DONE(d)")
          (sequence "SKETCH(s)" "DETAIL(D)" "WRITE-UP(w)" "|" "COMPLETED(c)")
          ("INVESTIGATE(i)" "REPORT(r)"   "|" "FINISHED(f)")
          (sequence "|" "CANCELED(C)")))
...
(setq pale-blue   '"#BBCCEE"
  ...
  (setq org-todo-keyword-faces
        '(("TODO" :background "#EEEEBB" :weight semi-bold)
          ("NEXT" :background "#FFCCCC" :weight bold)
          ("DONE" :background "#CCDDAA":weight semi-bold)
          ("SKETCH" :background "#BBCCEE" :weight normal)
          ("DETAIL" :background "#CCEEFF" :weight normal)
          ("WRITE-UP" :background "#EEEEBB" :weight normal)
          ("COMPLETED" :background "#DDDDDD" :weight normal)
          ("INVESTIGATE" :background "#BBCCEE" :weight normal)
          ("REPORT" :background "#CCEEFF" :weight normal)
          ("FINISHED" :background "#DDDDDD" :weight normal)
          ("CANCELED" :background "#DDDDDD" :weight normal)))
...
(use-package org-agenda
  :hook (org-agenda-finalize-hook . my-org-agenda-mode-hook)
  :config
...
We activate hl line mode in agenda buffers.
#+begin_src emacs-lisp
...
(use-package org-super-agenda
  ...
  (setq org-super-agenda-groups
       '(;; Each group has an implicit boolean OR operator between its selectors.
         (:name "Today"           ; section name
                :time-grid t      ; Items that appear on the time grid
                :scheduled today
                :deadline today)
         (:name "Outlook"
                :and (:scheduled future :priority>= "B")
                :and (:deadline future :priority>= "B")
                )
         (:name "Next"
                :todo "NEXT")
         (:name "Short"
...
(defun my-org-agenda-highlight-todo (x)
  (let* ((done (string-match-p (regexp-quote ":DONE:") x))
         ...
         (x (replace-regexp-in-string "~" "if canceled" (propertize x 'face 'nano-faded) x)))
    x))
...
#+RESULTS:
: ((~/Documents/org-files/agenda/maths.org :maxlevel . 5) (~/Documents/org-files/agenda/leisure.org :maxlevel . 5) ())
...
(setq org-capture-templates
      `(("i" "Inbox" entry  (file+headline "./agenda/inbox.org" "Miscellaneous")
        ,(concat "* TODO %?\n"
                 "/Entered on/ %U"))
        ("m" "Meeting" entry  (file+headline "./agenda/inbox.org" "Meetings")
        ,(concat "* %? :meeting:\n"
                 "SCHEDULED: %(org-insert-time-stamp (org-read-date nil t \"+1d\"))"))
        ("n" "Note" entry  (file+headline "./agenda/inbox.org" "Notes")
        ,(concat "* Note (%a)\n"
                 "/Entered on/ %U\n" "\n" "%?"))
        ("@" "Inbox [mu4e]" entry (file+headline "./agenda/inbox.org" "Mails")
...
(use-package org-gcal
  ...
  (setq org-gcal-client-id         "609704434601-f52c59f8oo68jg4nfkkrbhfrf3senhbl.apps.googleusercontent.com"
        ...
        org-gcal-fetch-file-alist '(("sebastianhalbig77@googlemail.com" .  "~/Documents/org-files/calendar.org"))
