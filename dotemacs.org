...
(use-package orderless
  ...
  :init
  (defun first-prefixes (pattern index _total)
    "The first input is treated as a prefix"
    (if (= index 0) 'orderless-prefixes))

  (defun initialism-if-hash (pattern _index _total)
    "Strings starting in # are matched wrt the initialism style"
    (cond
     ((equal "#" pattern)
      '(orderless-literal . ""))
  ...
  (defun flex-if-twiddle (pattern _index _total)
    "Strings starting in ~ are matched wrt the flex style"
    (cond
     ((equal "~" pattern)
      '(orderless-literal . ""))
     ((string-prefix-p "~" pattern)
  ...
  (defun without-if-bang (pattern _index _total)
    "Matches  of strings starting with ! are excluded from the results"
    (cond
     ((equal "!" pattern)
      '(orderless-literal . ""))
     ((string-prefix-p "!" pattern)
